...
// All external AI/search calls go through the auth-gated backend proxy.
// No third-party API keys ship in the client bundle.
...
Use types: LEARNING, CERTIFICATION, PROJECT, or APPLICATION. Be domain-specific and practical.`;
    ...
    const response = await fetch(`${API_BASE_URL}/roadmaps/ai-generate`, {
      method: 'POST',
      credentials: 'include',
    ...
    const generatedText: string = data?.text || '';
...
async function fetchRealJobData(input: SimulationInput): Promise<{ pathways: Pathway[]; totalResults: number }> {
  try {
    ...
    const response = await fetch(`${API_BASE_URL}/roadmaps/jobs-search`, {
      method: 'POST',
      credentials: 'include',
      headers: backendHeaders(),
      body: JSON.stringify({ query, page: 1, numPages: 1 }),
    });

    if (!response.ok) {
      throw new Error(`Jobs proxy responded with status: ${response.status}`);
    }
