...
function init() {
    ...
    autoStartFromRoadmapContext();
    
    console.log('App initialized successfully');
}

function autoStartFromRoadmapContext() {
    const params = new URLSearchParams(window.location.search);
    const shouldAutoStart = params.get('autoStart') === '1';
    if (!shouldAutoStart) return;

    try {
        const raw = localStorage.getItem('careersync_skill_eval_context_v1');
        if (!raw) return;

        const context = JSON.parse(raw);
        const skills = Array.isArray(context.skills) ? context.skills.filter(Boolean) : [];
        const resumeText = (context.resumeText || '').toString();

        autoEvalContext = {
            skills,
            targetRole: context.targetRole || '',
            resumeText,
            questionCount: Number(context.questionCount) || 20,
            difficulty: context.difficulty || 'intermediate'
        };

        const topicParts = [];
        if (autoEvalContext.targetRole) topicParts.push(autoEvalContext.targetRole);
        if (skills.length > 0) topicParts.push(skills.join(', '));
        courseInput.value = topicParts.join(' | ').slice(0, 220);
        ...
        const uiDifficulty = difficultyMap[autoEvalContext.difficulty] || 'medium';
        handleDifficultyClick(uiDifficulty);

        // Start automatically after UI state has settled.
        setTimeout(() => startTest(), 250);
    } catch (error) {
        console.warn('Failed to auto-start evaluator from roadmap context:', error);
    }
}
...
async function startTest() {
    ...
    try {
        ...
        const questions = await generateQuestions(courseName, backendDifficulty, autoEvalContext);
...
async function generateQuestions(courseName, difficulty, context = null) {
    const contextText = context
        ? [
            context.targetRole ? `Target Role: ${context.targetRole}` : '',
            Array.isArray(context.skills) && context.skills.length ? `Skills: ${context.skills.join(', ')}` : '',
            context.resumeText ? `Resume Extract: ${context.resumeText.slice(0, 4000)}` : ''
        ].filter(Boolean).join('\n')
        : '';
