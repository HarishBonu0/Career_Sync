// Career Sync Profile Utilities - Enhanced with Backend Integration
...
function getUserId() {
    const user = JSON.parse(localStorage.getItem('Career Sync_user') || '{}');
    return user._id || user.id || localStorage.getItem('Career Sync_userId');
}

// Helper to get user email from localStorage
function getUserEmail() {
    const user = JSON.parse(localStorage.getItem('Career Sync_user') || '{}');
    return user.email || localStorage.getItem('Career Sync_userEmail');
}

// Save course to profile (backend + localStorage)
async function saveCourseToProfile(courseData) {
    const enrolledCourses = JSON.parse(localStorage.getItem('Career Sync_enrolled_courses') || '[]');
    ...
    localStorage.setItem('Career Sync_enrolled_courses', JSON.stringify(enrolledCourses));
...
async function saveEvaluationToProfile(evaluationData) {
    const completedEvaluations = JSON.parse(localStorage.getItem('Career Sync_evaluations') || '[]');
    ...
    localStorage.setItem('Career Sync_evaluations', JSON.stringify(completedEvaluations));
...
async function getProfileData() {
        ...
        if (userId) {
            ...
            if (response.ok) {
                ...
                if (backendData.courses) {
                    localStorage.setItem('Career Sync_enrolled_courses', JSON.stringify(backendData.courses));
                }
                if (backendData.roadmaps) {
                    localStorage.setItem('Career Sync_saved_roadmaps', JSON.stringify(backendData.roadmaps));
                }
                if (backendData.evaluations) {
                    localStorage.setItem('Career Sync_evaluations', JSON.stringify(backendData.evaluations));
                }
    ...
    return {
        courses: JSON.parse(localStorage.getItem('Career Sync_enrolled_courses') || '[]'),
        roadmaps: JSON.parse(localStorage.getItem('Career Sync_saved_roadmaps') || '[]'),
        evaluations: JSON.parse(localStorage.getItem('Career Sync_evaluations') || '[]')
    };
}

// Export functions for use in other pages
if (typeof window !== 'undefined') {
    window.Career SyncProfile = {
